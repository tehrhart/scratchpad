...
def main():
	if len(sys.argv)!=2:
		...
		ans, unans = srp(packets, iface=tap_interface, timeout=3, inter=0.002)

		print "\nVLAN\tMAC Address\t\tDHCP Server IP"
		for p in ans:
			if Dot1Q in p[1]:
				print "%d\t%s\t%s" % (p[1][Ether][Dot1Q].vlan, p[1][Ether].src, p[1][IP].src)
			else:
				print "(none)\t%s\t%s" % (p[1][Ether].src, p[1][IP].src)	
